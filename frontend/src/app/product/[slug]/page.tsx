...
import { getProduct, getProductStores } from '@/lib/api';
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const stores = await getProductStores(slug);

  const specGroups = [
    { group: 'الشاشة', rows: [['الحجم', '6.7 بوصة OLED'], ['معدل التحديث', '120Hz']] },
    { group: 'المعالج والذاكرة', rows: [['المعالج', product.chip], ['الذاكرة / التخزين', '8GB / 256GB']] },
    { group: 'الكاميرا والبطارية', rows: [['الكاميرا الرئيسية', '48 ميجابكسل'], ['البطارية', product.battery]] },
  ];

  return (
    <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-start gap-8 px-4 py-6 pb-28 sm:px-6 md:grid-cols-[1fr_400px]">
      <div>
        <div className="mb-3.5 text-[13px] text-muted">
          <Link href="/" className="hover:text-green">الرئيسية</Link> / {product.brand} / {product.name}
        </div>
              ...
              <tbody>
                {stores.map((s) => (
                  <tr key={s.name} className={[s.best ? 'bg-green-tint' : '', !s.inStock ? 'text-[#aab3af]' : ''].join(' ')}>
                    ...
                    <td dir="ltr" className="num border-b border-line px-3 py-3 text-[15px] font-semibold">{s.price}</td>
                    <td className={['border-b border-line px-3 py-3 text-[13px]', s.inStock ? 'font-semibold text-green' : 'text-[#aab3af]'].join(' ')}>{s.avail}</td>
                    <td dir="ltr" className="num border-b border-line px-3 py-3 text-[13px] text-muted">{s.updated}</td>
                    <td className="border-b border-line px-3 py-3">
                      <a href={`/api/go/${product.id}`} className="rounded-[7px] bg-green px-3.5 py-1.5 text-[13px] font-semibold text-white hover:bg-green-deep">اذهب →</a>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <Block title="تاريخ السعر">
          ...
          <div className="mt-3 text-sm font-semibold">
            أدنى سعر مسجّل: <b dir="ltr" className="num">{product.lowest}</b> ر.س في ١٢ مارس.
          </div>
        </Block>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3.5 rounded-card border border-line bg-surface p-[18px]">
          ...
          <input dir="ltr" defaultValue={product.lowest} className="num h-[42px] w-28 rounded-field border border-line px-3 text-center text-[15px]" />
      ...
      <aside className="rounded-[14px] border border-line bg-paper p-[22px] md:sticky md:top-20">
        ...
        <div dir="ltr" className="num text-[38px] font-semibold leading-tight text-green">{product.from}</div>
        <div className="mb-4 text-[13px] text-muted">متجر التقنية · محدّث قبل ٣ د</div>
        <a href={`/api/go/${product.id}`} className="mb-2 block w-full rounded-field bg-green py-3 text-center text-sm font-semibold text-white hover:bg-green-deep">اذهب إلى المتجر</a>
