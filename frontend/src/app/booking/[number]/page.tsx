...
function Steps({ step }: { step: number }) {
  const items: [string, number][] = [['اختيار', 1], ['تأكيد', 2], ['استلام', 3]];
  return (
...
/**
 * Booking flow: choose store/specs -> confirm -> hold. The 24h hold (per the
 * SRS, overriding the brief mock's 48h copy) is enforced by the `Countdown`
 * seed below and the warning copy throughout.
 */
export default function Booking({ params }: { params: Promise<{ number: string }> }) {
  const { number: id } = use(params);
  const router = useRouter();
  const product = findProduct(id);
  const [phase, setPhase] = useState<'step1' | 'locking' | 'step2' | 'step3'>('step1');
    ...
    <div className="mx-auto max-w-2xl px-4 py-7 pb-28 sm:px-6">
        ...
        <div className={card}>
          ...
          <div className="mb-4">
            ...
            {[['متجر التقنية', 'حي العليا، الرياض · 2.4 كم', product.from], ['مكتبة جرير', 'طريق الملك فهد · 5.1 كم', product.p2]].map(([n, a, p], i) => (
              <button key={n} onClick={() => setStore(i)} className={['mb-2.5 flex w-full items-center gap-3.5 rounded-xl border-[1.5px] p-3.5 text-right', store === i ? 'border-green bg-green-tint' : 'border-line'].join(' ')}>
                ...
                <span dir="ltr" className="num text-[17px] font-semibold text-green">{p} ر.س</span>
              </button>
            ))}
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex w-fit items-center overflow-hidden rounded-field border border-line">
              ...
              <span dir="ltr" className="num w-12 border-x border-line text-center font-semibold leading-10">{cap}</span>
      ...
      {phase === 'step3' && (
        <div className={`${card} text-center`}>
          ...
          <div dir="ltr" className="num text-[26px] font-semibold tracking-wide">BK-7F42Q</div>
          ...
          <button onClick={() => router.push('/account/bookings')} className="mb-2 w-full rounded-field bg-green py-3 text-sm font-semibold text-white hover:bg-green-deep">عرض حجوزاتي</button>
