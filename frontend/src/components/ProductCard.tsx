    ...
    <div className="relative flex flex-col gap-1.5 rounded-card border border-line bg-paper p-3.5 transition-colors hover:border-[#c4cec9]">
      <button
        ...
        className={[
          ...
          isFav ? 'border-green-tint bg-green-tint text-green' : 'border-line bg-white/90 text-muted hover:text-ink',
        ].join(' ')}
      >
        ♥
      </button>
      ...
      <Link href={`/product/${product.id}`} className="text-[14.5px] font-semibold leading-6 text-ink hover:text-green">
        {product.name}
      </Link>
      <div className="text-[11.5px] text-muted">ابتداءً من</div>

      <div className="flex items-center gap-2">
        <span dir="ltr" className="num text-[22px] font-semibold">{product.from}</span>
