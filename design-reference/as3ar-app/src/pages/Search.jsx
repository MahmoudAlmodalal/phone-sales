...
import { PRODUCTS, byDrop, byPrice } from '../data/products.js'
...
const STORES = ['متجر التقنية', 'مكتبة جرير', 'إكسترا']

/** Checkbox row (square, green when checked). */
function Check({ label, checked, onChange }) {
  return (
    <label className="flex cursor-pointer items-center gap-2.5 py-1 text-[13.5px]" onClick={onChange}>
      <span
        className={[
 ...
 * booking-only) + live sort. Filters are interactive local state; the result
 * set re-sorts immediately. Product cards expose the compare toggle.
 */
export default function Search() {
  const [sort, setSort] = useState('low')
  const [brands, setBrands] = useState(['آبل', 'سامسونج'])
  ...
  return (
    <div className="mx-auto grid max-w-content grid-cols-1 items-start gap-6 px-4 py-6 pb-28 sm:px-6 md:grid-cols-[280px_1fr]">
      {/* Sidebar filters */}
      <aside className="overflow-hidden rounded-card border border-line bg-paper md:sticky md:top-20">
        <div className="border-b border-line p-4">
          ...
          <div className="num flex justify-between text-xs text-muted"><span>800</span><span>5,500</span></div>
        </div>
